...
client.on('ready', () => {
 ...
 client.user
      .setPresence({
        activity: {
          name: `Type ${config.prefix} to start`,
          type: "WATCHING",
        },
        status: "online",
      })
  ...
  if (await db.get(`tickets_${button.id}_${button.message.guild.id}`)) {
    ...
    if (button.values[0] === "men") { //admin +
          button.guild.channels.create(`ticket-${count}`, {
            ...
            ], parent: (await db.get(`Channels_${button.message.guild.id}.Cat`)), position: 1, topic: `A Ticket : <@!${button.clicker.user.id}>`, reason: "All rights reserved to MrVenomYt#7103"
          }).then(async channel => {
            channel = channel
